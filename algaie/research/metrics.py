from __future__ import annotations

from dataclasses import dataclass
from typing import Dict

import numpy as np
import pandas as pd


@dataclass(frozen=True)
class MetricsResult:
    equity: Dict[str, float]
    trades: Dict[str, float]


def compute_equity_metrics(equity_curve: pd.DataFrame, freq: int = 252) -> Dict[str, float]:
    if equity_curve.empty:
        return {}
    equity = equity_curve["equity"].astype(float)
    returns = equity.pct_change().dropna()
    total_return = equity.iloc[-1] / equity.iloc[0] - 1
    years = max(len(equity) / freq, 1e-6)
    cagr = (equity.iloc[-1] / equity.iloc[0]) ** (1 / years) - 1
    vol = returns.std(ddof=0) * np.sqrt(freq)
    downside = returns[returns < 0].std(ddof=0) * np.sqrt(freq)
    sharpe = returns.mean() / returns.std(ddof=0) * np.sqrt(freq) if returns.std(ddof=0) else 0.0
    sortino = returns.mean() / downside * np.sqrt(freq) if downside else 0.0
    drawdown = equity / equity.cummax() - 1
    max_drawdown = drawdown.min()
    drawdown_duration = _max_drawdown_duration(drawdown)
    calmar = cagr / abs(max_drawdown) if max_drawdown else 0.0
    ulcer_index = np.sqrt(np.mean(drawdown**2))
    skew = returns.skew() if not returns.empty else 0.0
    kurtosis = returns.kurtosis() if not returns.empty else 0.0
    return {
        "total_return": float(total_return),
        "cagr": float(cagr),
        "annualized_vol": float(vol),
        "downside_vol": float(downside),
        "sharpe": float(sharpe),
        "sortino": float(sortino),
        "max_drawdown": float(max_drawdown),
        "max_drawdown_duration": float(drawdown_duration),
        "calmar": float(calmar),
        "ulcer_index": float(ulcer_index),
        "skew": float(skew),
        "kurtosis": float(kurtosis),
    }


def compute_trade_metrics(trades: pd.DataFrame) -> Dict[str, float]:
    if trades.empty:
        return {}
    pnl = trades["pnl"].astype(float)
    wins = pnl[pnl > 0]
    losses = pnl[pnl <= 0]
    win_rate = len(wins) / max(len(trades), 1)
    profit_factor = wins.sum() / abs(losses.sum()) if losses.sum() else 0.0
    avg_win = wins.mean() if not wins.empty else 0.0
    avg_loss = losses.mean() if not losses.empty else 0.0
    payoff = avg_win / abs(avg_loss) if avg_loss else 0.0
    expectancy = pnl.mean()
    trade_days = trades[["exit_date"]].drop_duplicates().shape[0]
    trades_per_day = len(trades) / trade_days if trade_days else 0.0
    max_wins, max_losses = _max_consecutive(pnl)
    return {
        "win_rate": float(win_rate),
        "profit_factor": float(profit_factor),
        "avg_win": float(avg_win),
        "avg_loss": float(avg_loss),
        "payoff_ratio": float(payoff),
        "expectancy": float(expectancy),
        "median_trade_return": float(trades["ret"].median()),
        "trades": float(len(trades)),
        "trades_per_day": float(trades_per_day),
        "max_consecutive_wins": float(max_wins),
        "max_consecutive_losses": float(max_losses),
    }


def compute_metrics(equity_curve: pd.DataFrame, trades: pd.DataFrame) -> MetricsResult:
    return MetricsResult(
        equity=compute_equity_metrics(equity_curve),
        trades=compute_trade_metrics(trades),
    )


def _max_drawdown_duration(drawdown: pd.Series) -> int:
    duration = 0
    max_duration = 0
    for value in drawdown:
        if value < 0:
            duration += 1
            max_duration = max(max_duration, duration)
        else:
            duration = 0
    return max_duration


def _max_consecutive(pnl: pd.Series) -> tuple[int, int]:
    max_wins = 0
    max_losses = 0
    current_wins = 0
    current_losses = 0
    for value in pnl:
        if value > 0:
            current_wins += 1
            current_losses = 0
        else:
            current_losses += 1
            current_wins = 0
        max_wins = max(max_wins, current_wins)
        max_losses = max(max_losses, current_losses)
    return max_wins, max_losses

...
from algaie.data.options.greeks_engine import bs_delta, bs_gamma, bs_price, bs_theta, bs_vega
...
class VRPStrategy:
    ...
    def compute_features(
        ...
        surface = IVSurfaceBuilder.build_snapshot(chain)
    ...
    def _build_put_credit_spread(
        ...
        T = dte / 365.0
        r = float(puts["risk_free_rate"].iloc[0])
        q = float(puts["dividend_yield"].iloc[0])

        # Compute deltas for all puts
        puts = puts.copy()
        puts["_delta_abs"] = np.abs(
            bs_delta(underlying_price, puts["strike"].to_numpy(), T, r,
        ...
        # Greeks at entry
        short_delta = float(bs_delta(underlying_price, short_strike, T, r, short_iv, "put", q))
        long_delta = float(bs_delta(underlying_price, long_strike, T, r, long_iv, "put", q))
        short_gamma_v = float(bs_gamma(underlying_price, short_strike, T, r, short_iv, q))
        long_gamma_v = float(bs_gamma(underlying_price, long_strike, T, r, long_iv, q))
        short_vega_v = float(bs_vega(underlying_price, short_strike, T, r, short_iv, q))
        long_vega_v = float(bs_vega(underlying_price, long_strike, T, r, long_iv, q))
        short_theta_v = float(bs_theta(underlying_price, short_strike, T, r, short_iv, "put", q))
        long_theta_v = float(bs_theta(underlying_price, long_strike, T, r, long_iv, "put", q))
    ...
    def check_exits(
        ...
        if regime in (VolRegime.CRASH_RISK, VolRegime.CAUTION):
            ...
            if remaining and underlying_prices:
                ...
                from algaie.execution.options.structures import DerivativesPositionFrame

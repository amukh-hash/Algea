"""
Institutional Optimization: StatArb iTransformer + Huber Loss + Embargo CV
Target: cuda:0 (RTX 3090 Ti). Estimated burn: ~2-4 hours.

The iTransformer uses "inverted" attention — each variate (ETF) is a token,
and self-attention learns cross-variate lead-lag dependencies.

Target: Forward 12-bar Δ Z-score per ETF (mean-reversion signal).
        y = Z_{t+12} - Z_t  (NOT raw price → prevents identity mapping)

60-minute bars: lookback=60 → 8.5 trading days, 12-bar target → 1.7 days.
This steps over the HFT microwave boundary into institutional VWAP rotation.

Loss: HuberLoss (δ=1.0) — caps extreme gradient from macro shocks.
Gate: Directional Accuracy > 51% (correctly predicts sign of spread move).
...
DATA_DIR = Path("data_lake/statarb_60min")
...
if __name__ == "__main__":
    logger.info("Sequence 3R v2: StatArb iTransformer 60-min — 40 trials × 5 folds")

    X_raw = np.load(DATA_DIR / "X_features_60min.npy")
    y_raw = np.load(DATA_DIR / "y_targets_60min.npy")

...
import { ButtonHTMLAttributes, PropsWithChildren, ReactNode, useMemo, useState } from "react";
...
export function SearchInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className={cn("rounded-md border border-border bg-surface-2 px-3 py-2 text-sm", props.className)} />;
}
      ...
      <div role="tablist" aria-label="Run detail tabs" className="flex gap-2 border-b border-border pb-2">
        {items.map((tab, i) => (
          <button
            ...
            className={cn("rounded px-3 py-1.5 text-sm", tab.id === active ? "bg-surface-2" : "text-secondary")}
          >{tab.label}</button>
        ))}
      </div>

...
def load_ohlcv(symbol, start_date=None, end_date=None, required_cols=None, target_col=None) -> pd.DataFrame:
    paths = pathmap.get_paths()
    ...
    if required_cols:
        ...
        if missing: raise ValueError(f"Missing required columns {missing} in file {path}")
    if target_col:
        if target_col not in df.columns: raise ValueError(f"{target_col} missing in file {path}")
        target = pd.to_numeric(df[target_col], errors="coerce")
        if target.isna().all(): raise ValueError(f"{target_col} contains only nulls in file {path}")
        if target.nunique(dropna=True) <= 1: raise ValueError(f"{target_col} is constant in file {path}")
    if start_date: df = df[df["date"] >= pd.Timestamp(start_date)]
    if end_date: df = df[df["date"] <= pd.Timestamp(end_date)]
    return df

...
from algae.trading.meta_allocator import SleeveResult
...
class COOCReversalFuturesSleeve:
    ...
    - There is no global BaseSleeve class yet; production sleeves are strategy classes
      (e.g. ``algae.execution.options.vrp_strategy.VRPStrategy``).
    - Meta allocation currently passes ``SleeveResult`` objects into
      ``algae.trading.meta_allocator.MetaAllocator.combine``.
    - Regime states come from ``VolRegime`` enum: NORMAL_CARRY/CAUTION/CRASH_RISK.
    - Model registry exists under ``algae.models.foundation.registry``.
    - Broker abstraction exists at ``algae.trading.broker_base`` with concrete IBKR adapters.
    """

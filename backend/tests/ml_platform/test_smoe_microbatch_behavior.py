...
def test_smoe_microbatch(tmp_path: Path):
    ...
    svc = SMoEService(SMoELoader(store), cfg.trace_root, microbatch_size=2)
    out = svc.rank(SMoERankRequest(asof="2026-01-01", symbols=["A", "B", "C"], feature_matrix=[[0.1], [0.2], [0.3]], trace_id="mb", market_context={}))

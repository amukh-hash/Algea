...
def test_trace_contains_smoe_fields(tmp_path: Path):
    ...
    svc.rank(SMoERankRequest(asof="2026-01-01", symbols=["A"], feature_matrix=[[0.1]], trace_id="trace-smoe", market_context={}))
    payload = json.loads((cfg.trace_root / "trace-smoe.json").read_text(encoding="utf-8"))
    assert "router_entropy_mean" in payload
    assert "expert_utilization" in payload
